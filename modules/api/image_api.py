...
def api_v2_song_record(user_id, song_id):
    ...
    played_data = [
        ...
        if record['cover_name'] == matching_song['cover_name'] and record['type'] == matching_song['type']
    ]
    if not played_data:
...
def api_v2_generate_record_image(user_id):
    ...
    first_word, _, rest_text = command.partition(" ")
    ver = _udata.get("version", "jp")
    record_type = next((
        mode for aliases, mode in RANK_COMMANDS.items()
        if first_word in (aliases if isinstance(aliases, tuple) else (aliases,))
    ), None)
    if not record_type:
        available = [alias for aliases in RANK_COMMANDS
                     for alias in (aliases if isinstance(aliases, tuple) else (aliases,))]
        return jsonify({"error": f"Unknown command: {command}", "available": available}), 400

    song_record = read_record(
        user_id,
        record_type == "rct50",
        record_type == "best40",
        ver=ver,
...
def api_v2_generate_plate(user_id):
    ...
    version_name, plate_type = title[0], title[1:]
    ...
    version_records = [record for record in song_record if record['version'] in target_version]
