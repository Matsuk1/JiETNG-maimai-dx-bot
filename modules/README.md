...
完整成绩识别先走本地定位、OCR 和校验。定位/裁切/引擎异常，或校验未通过且没有本地补全候选时，才使用已启用 AI Monitor 的 Codex 登录尝试一次原图识别。Codex OCR 默认使用 `gpt-5.6-luna` 与 `low` 推理强度；可通过 `JIETNG_CODEX_OCR_MODEL` 或 `ai_monitor.ocr_model` 单独更换模型，不继承后台诊断模型。Codex OCR 使用独立临时会话，不启用后台 MCP、网页或 shell 工具；结果必须再次通过谱面和达成率校验。未登录、不可用或结果不合格时保留原错误/手动修正流程。仅歌名识别、手动 `fix-rcd` 和裁切预览不触发保底。
