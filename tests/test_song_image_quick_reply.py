...
def test_song_images_keep_localized_actions():
    for mode, commands in [('info', ['search-record abc123']),
                           ('record', ['search-song abc123'])]:
        with patch('modules.messages.service.get_multilingual_text', return_value='Localized action'):
        ...
        assert [action['data'] for action in actions] == commands
...
def test_quick_reply_image_stays_after_appended_notice():
    ...
    with patch('modules.messages.service.get_multilingual_text', return_value='Info'):
    ...
    assert sent[-1].quick_reply.items[0].action.data == 'search-song abc123'
