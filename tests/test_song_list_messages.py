...
def test_pagination_visits_every_song_once(count):
    ...
    for page in range(1, total_pages + 1):
        ...
        found.extend(action for action in actions if action.startswith('search-song '))
    ...
    assert found == [f'search-song {i}' for i in range(count)]
...
def test_search_limit_and_record_owner_are_preserved():
    with patch.object(service, 'get_user_language', return_value='en'):
        message = service.generate_search_results_flex('user', songs(25), 'record', 'friend')
    assert commands(message) == [f'search-record {i}&id_use=friend' for i in range(20)]
