...
| 機能 | 説明 |
|------|------|
| 画像を送信（QRコード） | 画像内のQRコードを自動スキャンし、フレンド追加リンクやその他のデータを識別 |
| 画像を送信（カバー認識） | 画像内の楽曲カバーを認識し、楽曲情報と個人スコアを自動返信 |
...
**カバー認識説明**：
- 楽曲カバーを含む画像（ゲームスクリーンショット、プロモーション画像など）を送信
- Botが自動的にカバーを認識し、対応する楽曲情報を返信
- ハイブリッド認識戦略：
  - **ハッシュマッチング**：完全なカバーの高速認識（高精度）
  - **SIFT特徴点マッチング**：シーン画像内の部分カバー認識（高再現率）
- 1枚の画像で複数楽曲認識に対応（最大5曲）
- 認識率：完全なカバーはほぼ100%、部分的な遮蔽は状況により異なる
