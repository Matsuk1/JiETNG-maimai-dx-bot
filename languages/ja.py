...
TEXTS["message_manager"] = {'help_ui': {'b_subtitle': 'Best / All Best / 特殊成績画像とフィルター',
 ...
 'score_recognition': {'break_detail': 'BREAK 詳細判定',
                       ...
                       'calc_consistent_unverified': 'Calc の達成率と判定データは一致しています（譜面未定）',
...
TEXTS["main"] = {'account_already_bound': 'すでに SEGA アカウントが連携されています。再度連携する場合は、先に unbind コマンドで連携を解除してください。',
 ...
 'maimai_service_busy_body': 'maimai 公式サービスの応答が遅いか、アクセス制限が発生しています。しばらくしてからもう一度お試しください。',
 'no_linked_account': '連携済みアカウントがありません。',
 'not_linked_title': '未連携',
 'private_chat_title': '個人チャットで使用してください',
 'query_failed_title': '検索に失敗しました',
...
TEXTS["command_help"] = {'bind': '命令: bind\n'
 ...
 'refreshmenu': '命令: refreshmenu\n'
                ...
                '示例: refreshmenu',
 'score_recognition': '命令: rec [-flex]\n说明: ローカル OCR でリザルトを認識します。検証に成功すると結果画像を返し、補正に失敗すると元の認識データとコピー可能な修正コマンドを表示します。AI は自動で呼び出しません。AI 認識には ai-rec を使用してください。\n参数: リザルト画像への返信が必須です。-flex を付けると Flex カードで表示します。\n示例: rec\nrec -flex',
 'ai_score_recognition': '命令: ai-rec [-flex]\n说明: Codex AI で元のリザルト画像を認識し、検証に成功すると結果画像を返します。\n参数: リザルト画像への返信が必須です。-flex を付けると Flex カードで表示します。\n制限: AI 認識の利用権限が必要です。通常の rec は AI を呼び出しません。\n示例: ai-rec\nai-rec -flex',
 ...
 'song_info': '命令: [<曲名>] info\n'
              ...
              '参数: 曲名は省略可能です。省略時は空タイトルの楽曲を検索し、画像への返信時は曲名を自動認識します。\n'
              '検索: 複数候補がある場合は選択候補を返します。\n'
              '示例: ヒバナ info\n'
              '（画像に返信）info',
 'song_record': '命令: [<曲名>] record\n'
                '说明: 曲名または別名で自分の単曲成績を検索します。\n'
                '参数: 任意: <曲名>。省略時は空タイトルの楽曲を検索します。\n'
                '検索: 複数候補がある場合は選択候補を返します。\n'
                '示例: ヒバナ record',
 'status': '命令: status\n'
